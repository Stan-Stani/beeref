...
* Images now remember where they were pulled from. When an image is added
  from a file, dragged in from the web, or pasted from a browser, its
  origin (file path or URL) is saved with the image, shown on hover, and
  can be copied via Images -> Copy Image Source. For web drags this is the
  page/link you dragged, so there's a record of the source even when the
  image itself was taken from the dropped data; for pastes the source is
  read from the ``<img>`` link browsers put on the clipboard.
